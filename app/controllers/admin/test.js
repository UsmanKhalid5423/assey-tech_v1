...
const fetch = async (req,res,next)=>{
    ...
    try {
        let query = {}
        const {testStatus,id} = req.query

        if(testStatus)
        {
            query['timeStatus'] = testStatus
        }
        if(id)
        {
            query['_id'] = id
        ...
        for(let i=0;i<testList.length;i++)
        {
            ...
            let labDetails = await database.findById_v2(models.lab,testList[i].labId)
 ...
 */
 const remove = async (req,res,next)=>{
    ...
    try {
        ...
        const testDetails = await database.remove(models.test,{ '_id': id });
        if(testDetails)
        {
