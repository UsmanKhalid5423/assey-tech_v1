...
Router.get("/api/drforgetPassword/", async (req, res) => {
    ...
    try {
        ...
        const user = await drAccounts.findOne({ email });
        if (user) {
            if (user.phone_number == phone) {
                const hashPass = await bcrypt.hash(newPassword, 10);
                const update = await drAccounts.findOneAndUpdate({ email }, {
                ...
                /////////////////////////////
                if (update) {
                    const newUser = await drAccounts.findOne({ email });
